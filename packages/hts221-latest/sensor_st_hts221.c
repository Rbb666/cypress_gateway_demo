 ...
 * Change Logs:
 * Date           Author       Notes
 * 2019-02-16     flybreak     first version
 */
...
static rt_err_t _hts221_init(struct rt_sensor_intf *intf)
{
    ...
    {
        LOG_E("Can not find dev[%s]", intf->dev_name);
        return -RT_ERROR;
    }

    /* Configure the humilero driver */
    io_ctx.BusType     = HTS221_I2C_BUS; /* I2C */
    io_ctx.Address     = (rt_uint32_t)(intf->arg) & 0xff;
    io_ctx.Init        = RT_NULL;
...
{
    if (power == RT_SENSOR_MODE_POWER_DOWN)
    {
    ...
    else
    {
        ...
        HTS221_TEMP_SetOutputDataRate(&hts221, 100);
    }
    return RT_EOK;
}
...
static rt_ssize_t hts221_fetch_data(struct rt_sensor_device *sensor, rt_sensor_data_t buf, rt_size_t len)
{
    ...
    else if (sensor->info.type == RT_SENSOR_TYPE_HUMI)
    {
        ...
        data->type = RT_SENSOR_TYPE_HUMI;
        data->data.humi = humi_value;
        data->timestamp = rt_sensor_get_ts();
    }
...
int rt_hw_hts221_humi_init(const char *name, struct rt_sensor_config *cfg)
{
    ...
    sensor->info.type       = RT_SENSOR_TYPE_HUMI;
    sensor->info.vendor     = RT_SENSOR_VENDOR_STM;
