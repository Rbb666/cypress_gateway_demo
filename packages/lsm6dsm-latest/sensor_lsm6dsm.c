...
{
    if (sensor->info.type == RT_SENSOR_TYPE_ACCE)
    {
    ...
    else if (sensor->info.type == RT_SENSOR_TYPE_GYRO)
    {
...
static rt_err_t lsm6dsm_set_mode(rt_sensor_t sensor, rt_uint8_t mode)
{
    ...
    break;
//    case RT_SENSOR_MODE_FETCH_NONE:
    default:
        break;
    }
...
static rt_err_t lsm6dsm_set_power(rt_sensor_t sensor, rt_uint8_t power)
{
    ...
    {
        if (sensor->info.type == RT_SENSOR_TYPE_GYRO)
        {
        ...
        {
            if (sensor->info.type == RT_SENSOR_TYPE_STEP)
            {
                if (lsm6dsm_disable_step_counter(12.5f))
                {
    ...
    case RT_SENSOR_MODE_POWER_MEDIUM:
    {
        if (sensor->info.type == RT_SENSOR_TYPE_GYRO)
        {
            ...
            lsm6dsm_odr_g_t new_odr = lsm6dsm_gyro_convert_odr(12.5f);
            if (lsm6dsm_gy_data_rate_set(&dev_ctx, new_odr))
            {
                LSM_ERR("Set gyro odr failed!");
                return -RT_ERROR;
        ...
        else
        {
            ...
            {
                lsm6dsm_odr_xl_t new_odr = lsm6dsm_acce_convert_odr(12.5f);
...
static rt_ssize_t lsm6dsm_polling_fetch_data(rt_sensor_t sensor, struct rt_sensor_data *data)
{
    ...
    else if (sensor->info.type == RT_SENSOR_TYPE_STEP)
    {
        uint16_t step = 0;
        data->type = RT_SENSOR_TYPE_STEP;
    ...
    else if (sensor->info.type == RT_SENSOR_TYPE_TEMP)
    {
...
static rt_size_t lsm6dsm_fetch_data(struct rt_sensor_device *sensor, void *buf, rt_size_t len)
{
    ...
    else if (sensor->info.mode == RT_SENSOR_MODE_FETCH_INT)
    {
        return 0;
    }
    else if (sensor->info.mode == RT_SENSOR_MODE_FETCH_FIFO)
    {
...
int rt_hw_lsm6dsm_init(const char *name, struct rt_sensor_config *cfg)
{
    ...
    sensor_acce.info.type       = RT_SENSOR_TYPE_ACCE;
    ...
    sensor_acce.info.acquire_min = 10;
...
//    cfg->odr = cfg->odr < LSM6DSM_ODR_MIN ? LSM6DSM_ODR_MIN : cfg->odr;
    ...
    sensor_gyro.info.type       = RT_SENSOR_TYPE_GYRO;
    ...
    sensor_gyro.info.acquire_min = 10;
...
//    sensor_step.info.type       = RT_SENSOR_TYPE_STEP;
...
//    sensor_step.info.acquire_min = 10;
...
//#endif /*PKG_LSM6DSM_USING_STEP*/
    ...
    sensor_temp.info.acquire_min = 10;
