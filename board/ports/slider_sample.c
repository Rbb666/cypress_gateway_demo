...
static void process_touch(void)
{
    ...
    if (touch_prev_flag - touch_flag == 1 && rt_wlan_is_connected() && onenet_sync_flag)
    {
        rt_kprintf("touch down, brightness is %d\n", led_data.brightness);
        touch_prev_flag = 0;

        if (onenet_mqtt_upload_digit("Led_pwm", led_data.brightness) < 0)
            rt_kprintf("not connect one-net,upload error\n");
    }
